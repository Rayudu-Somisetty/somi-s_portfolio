...
- [x] Update all section components (`HeroSection`, `AboutSection`, `SkillsSection`, `ProjectsSection`, `InternshipsSection`, `CertificationsSection`, `ContactSection`) to use the shared spacing utility instead of inconsistent `py-*`/`min-h-screen`.
- [x] Ensure sections keep their intended layout (avoid breaking hero min-height).
...
## Step 4 (Verification)
- [x] Run `npm test` / `npm run build` (or at least `npm run lint` if available).
- [x] Manual visual check: hero CTA readability, spacing consistency, navigation label clarity.